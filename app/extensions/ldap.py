...
def authenticate(user, password):
    # define the server
    s = Server(
        current_app.config["LDAP_HOST"], get_info=ALL
...
def get_dn_from_group(group):
    return "%s=%s,%s" % (
        current_app.config["LDAP_GROUP_ATTRIBUTE"],
        group,
        current_app.config["LDAP_GROUP_BASE"],
    )
...
class TedivmLoginForm(LoginForm):
        ...
        if user_manager.USER_ENABLE_USERNAME:
            ...
            if not user and user_manager.USER_ENABLE_EMAIL:
                (
                    user,
                    user_email,
                ) = user_manager.db_manager.get_user_and_user_email_by_email(
                    self.username.data
                )
    ...
    def validate(self):
        ...
        message = _(
            "Incorrect %(username_or_email)s and/or Password",
